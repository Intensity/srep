  ...
  if (2*L <= block_size)
  {
    ...
    for (NUMBER i=0; i<=block_size-2*L; )
    {
      ...
      for (NUMBER next_chunk = i+L;  likely(i<next_chunk);  )
      {
        ...
        if (i >= match_start)
        {
          // Encode input match if it has at least BASE_LEN bytes after removal of bytes overlapped with our own one
          if (match_start+match_len-BASE_LEN >= last_match_end)
          {
            match_len  -= mymax(match_start, last_match_end) - match_start;
            match_start = mymax(match_start, last_match_end);
