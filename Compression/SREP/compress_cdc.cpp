...
struct CDC_Global
{
  ...
  CDC_Global (bool _CONTENT_DEFINED_CHUNKING, int NumThreads)
  {
    CONTENT_DEFINED_CHUNKING = _CONTENT_DEFINED_CHUNKING;  errcode = NO_ERRORS;
    if (!CONTENT_DEFINED_CHUNKING)  return;
  ...
  ~CDC_Global()  {if (CONTENT_DEFINED_CHUNKING && errcode==NO_ERRORS)  Threads.finish();}
};
